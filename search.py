...
def graph_search(problem, fringe, node_g, node_v):
    ...
    while fringe:
        print(f"salida graph_search {(node_g, node_v)}")
        node = fringe.pop()
        node_v += 1
        ...
        if node.state not in closed:
            ...
            node_g += len(node.expand(problem))
    return None


def breadth_first_graph_search(problem):
    """Search the shallowest nodes in the search tree first. [p 74]"""
    return graph_search(problem, FIFOQueue(), 0, 0)  # FIFOQueue -> fringe


def depth_first_graph_search(problem):
    """Search the deepest nodes in the search tree first. [p 74]"""
    return graph_search(problem, Stack(), 0, 0)
